    ...
    <nav className="sticky top-0 z-50 border-b border-black/5 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-10">
        ...
        <div className="hidden items-center gap-2 md:flex">
          ...
          {role === "business" && (
            <Link href="/business/dashboard" className={linkClass("/business/dashboard")}>
              Dashboard
            </Link>
          )}
          ...
          {!user && (
            <Link href="/business/register" className={linkClass("/business/register")}>
              For Business
            </Link>
          )}
        </div>
      ...
      {mobileMenu && (
        <div className="border-t border-gray-100 bg-white px-5 py-5 md:hidden">
          <div className="grid gap-3">
            <Link href="/offers" onClick={() => setMobileMenu(false)} className={linkClass("/offers")}>
              Offers
            </Link>

            {user && (
              <Link href="/orders" onClick={() => setMobileMenu(false)} className={linkClass("/orders")}>
                Orders
              </Link>
            )}
            ...
            {role === "admin" && (
              <Link href="/admin" onClick={() => setMobileMenu(false)} className={linkClass("/admin")}>
                Admin
              </Link>
            )}
